...
class SpatialNavigation extends Component {
    constructor(props) {
        ...
        config =this.getConfigFromProps.call(this);
        const spatialConfig = Object.assign(defaultConfig, config);

        if (!this.props.customInit) {
            JsSpatialNavigation.init();
            JsSpatialNavigation.add(spatialConfig);
        } else {
...
class FocusableSection extends Component {
    sectionId = JsSpatialNavigation.add(config, this.props.sectionId);
